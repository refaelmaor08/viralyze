...
import { analyzeAdaptive } from '@/lib/aiProvider';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await analyzeAdaptive(frameData, context, understanding);
    return NextResponse.json(result);
  } catch (error) {
    const isDev = process.env.NODE_ENV === 'development';
    const message = error instanceof Error ? error.message : String(error);
    console.error('[Viralyze] Adaptive analysis error:', error);
    return NextResponse.json(
      {
        error: isDev ? `שגיאת Adaptive Analysis: ${message}` : 'שלב הניתוח נכשל.',
