...
import { analyzeTimeline } from '@/lib/aiProvider';
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await analyzeTimeline(frameData, context, understanding);
    return NextResponse.json(result);
  } catch (error) {
    const isDev = process.env.NODE_ENV === 'development';
    const message = error instanceof Error ? error.message : String(error);
    console.error('[Viralyze] Timeline analysis error:', error);
    return NextResponse.json(
      {
        error: isDev ? `שגיאת Timeline: ${message}` : 'שלב הניתוח נכשל.',
